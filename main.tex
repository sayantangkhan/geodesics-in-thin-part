...
\item ({\color{red}TODO}) Fix the above definition with an additional parameter.
...
\begin{proof}[Proof of Theorem \ref{thm:weak-convexity}]
  ...
  The first step of our proof is modifying the path $[x,y]$ and .
  We do so by constructing new points $p_i^{\prime}$, where $p_i^{\prime}$ is obtained from $p_i$ be increasing the length of any one-sided curve that's shorter than $\vept$ to $\vept$.
