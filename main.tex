...
We will prove a statement that is stronger than statistical convexity of $\thick(\no_g)$, namely that the entropy for $\alpha$-bad points is strictly lower than lattice point entropy, for any value of $\alpha < 1$.

\begin{theorem}[Statistical convexity]
  \label{thm:stat-convex}
  For any $\alpha < 1$, and any $\vept > 0$, the following inequality holds.
  \begin{align*}
...
\end{itemize}
  ...
  \item Proof for $x \in R_2$: In this case, let $\left\{ \gamma_1, \ldots, \gamma_k \right\}$ be the two-sided curves that get short at some point in $B_{\tau}^{\vept}(x)$.
    ...
    We have that there exists some constant $c(g)$, depending only on $g$, such that $k \leq c(g)$, since we cannot have too many curves being short simultaneously.
    ...
    \begin{align*}
      \frac{\flm^{\prime}(x)}{c(g)} \leq \flm(x) \leq \flm^{\prime}(x)
    \end{align*}
    ...
    \begin{align*}
      (A_{\tau}\flm)(x) \leq  (c(g) \cdot c^{\prime}) \tau \exp(-\tau) \flm(x)
    \end{align*}
  ...
  \end{align*}
  Here, $c(x) \coloneqq (c(g) \cdot c^{\prime}) \tau \exp(-\tau)$, and $b(x)$ is the function from the proof in case $R_3$.
\end{proof}
