...
\begin{lemma}
  ...
  There exists a polynomial function $q$, such that for any $\gamma \in \mcg(S)$, the following inequality holds for the cardinality of points in $N(\gamma)$.
  \begin{align*}
...
\begin{proof}[Proof of Lemma \ref{lem:fd-polynomial-growth}]
  ...
  It will suffice to construct a set $\cZ \subset \teich(S)$ such that $\# \cZ \leq R^{f(\chi(S))}$, and for $y \in B_R(p)$, there exists a $z \in \cZ$ and $\kappa \in \mcg(S)$ such that $d(y, \kappa z) \leq C$, for some function $f$, and some fixed constant $C$.
