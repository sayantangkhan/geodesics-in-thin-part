...
\item[-] $\hNP(\mathrm{core}(\teich(S)))$: This is the exponential growth rate for the net points in an $(\vepn, 2 \vepn)$-net in the weak convex core of $\teich(S)$. The value of $\vepn$ is usually clear from the context.
...
\begin{proof}
  ...
  \item $\os_{1,1,0}$: Since $\os_{1,1,0}$ is orientable, we have that $\mathrm{core(\teich(\os_{1,1,0}))} = \teich(\os_{1,1,0})$, so it suffices to look at the full Teichmüller space.
    ...
    \begin{align}
      ...
      \#\left( B_R(p) \cap \mathcal{N} \right) = \#\left( B_R(p) \cap \mathcal{N}_{\leq \vepb R} \right) + \#\left( B_R(p) \cap \mathcal{N}_{> \vepb R} \right)
    \end{align}
    Here, $\mathcal{N}_{\leq \vepb R}$ denotes the net points that lie within distance $\vepb R$ of the orbit of $p$, and $\mathcal{N}_{> \vepb R}$ denotes the net points that lie more than distance $\vepb R$ of the orbit of $p$.
    ...
    Let $\mathcal{N}_{\leq \vepb R}(\gamma)$ denote the subset of $\mathcal{N}_{\leq \vepb R}$ whose closest lattice point is $\gamma p$.
    ...
    \begin{align*}
      \#\left( B_R(p) \cap \mathcal{N}_{\leq \vepb R} \right) \leq p(R)
    \end{align*}
    Using the two facts we stated, we get the following upper bound for $\#\left( B_R(p) \cap \mathcal{N}_{\leq \vepb R} \right)$.
    \begin{align*}
      \#\left( B_R(p) \cap \mathcal{N}_{\leq \vepb R} \right) \leq p(R) \cdot \left( \exp(R(1+\vepb)) \right)
    \end{align*}
    ...
    For any point $x$ in $\mathcal{N}_{> \vepb R}$, we can replace the geodesic $[p, x]$ with two shorter segments, $[p, x_0]$ and $[x_0, x]$, where $x_0$ is the net point closest to the last point on the [p, x] which stays within some bounded distance of a lattice point.
    ...
    We thus have the following upper bound on $\#\left( B_R(p) \cap \mathcal{N}_{> \vepb R} \right) $ for large enough values of $R$.
    \begin{align*}
      \#\left( B_R(p) \cap \mathcal{N}_{> \vepb R} \right)  &\leq p(R) \cdot \exp(R(1-\vepb)) \\
...
\begin{definition}[Good points]
  A point in $B_R(p) \cap \mathcal{N}$ is good if it is at most distance $\vepb R$ away from a lattice point $\gamma p$. The set of good points is denoted by $\mathcal{N}_g(p, R, \vepb)$.
\end{definition}

\begin{definition}[Bad points]
  A point in $B_R(p) \cap \mathcal{N}$ is bad if it is more than distance $\vepb R$ away from the nearest lattice point. The set of bad points is denoted by $\mathcal{N}_b(p, R, \vepb)$.
\end{definition}
...
We also further subdivide $\mathcal{N}_g(p, R, \vepb)$ based on what the closest lattice point is.

\begin{definition}[Good point in the domain of $\gamma$]
  For $\gamma \in \mcg(S)$, the set $\mathcal{N}_g(\gamma, p, R, \vepb)$ denotes the subset of $\mathcal{N}_g(p, R, \vepb)$ whose closest lattice point is $\gamma p$.
\end{definition}
...
\begin{lemma}
  ...
  There exists a polynomial function $q$, whose degree only depends on the topological type of $S$, such that for any $\gamma \in \mcg(S)$, the following inequality holds for the cardinality of points in $\mathcal{N}_g(\gamma, p, R, \vepb)$.
  \begin{align*}
    \#\left( \mathcal{N}_g(\gamma, p, R, \vepb) \cap B_R(\gamma p) \right) \leq q(R)
  \end{align*}
\end{lemma}
...
Before we prove Lemma \ref{lem:fd-polynomial-growth}, we will need the following lemma on packing an $\vepn$-separated set into a ball of fixed radius in Teichmüller space (where $\vepn$ is the parameter associated to our net $\mathcal{N}$).
...
\begin{proof}[Proof of Lemma \ref{lem:fd-polynomial-growth}]
  ...
  \end{align}
  Here, $\mathcal{N}(\gamma)$ denotes the set of net points whose closest lattice point is $\gamma p$: $\mathcal{N}(\gamma)$ is therefore a superset of $\mathcal{N}_g(\gamma, p, R, \vepb)$.
...
\begin{theorem}[Theorem 12.1 of \cite{dowdall2023lattice}, Theorem \ref{thm:counting-with-complexity}]
  ...
  There exists a polynomial function $p(R)$ that depends on the net $\mathcal{N}$, and a parameter $\veperr > 0$, such that the following inequality holds for any $\veperr > 0$.
  \begin{align*}
    \#\left( y \in \mathcal{N} \mid \dcomp(p, x) \leq R \right) \leq p(R) \exp((\hNP(\mathrm{core}(\teich(S))) + \veperr) \cdot R)
  \end{align*}
\end{theorem}
...
\begin{theorem}[Linear gap in complexity length, Theorem \ref{thm:linear-gap}]
  ...
  \begin{align*}
    \hNP(\mathrm{core}(\teich(V))) < \hNP(\mathrm{core}(\os))
  \end{align*}
  Then for any $\vepb >0$, there exists $c > 0$, such that for any bad point $y$, i.e. a point in $\mathcal{N}_b(p, R, \vepb)$, the following upper bound on the complexity distance between $p$ and $y$ holds.
  \begin{align*}
    \dcomp(p, y) \leq R(1 - c)
  \end{align*}
\end{theorem}
...
\begin{proof}[Proof of Lemma \ref{lem:entropy-inequality}]
  ...
  We have that Theorem \ref{thm:entropy-equality} holds for $V$, which means that $\mathrm{core}(\teich(V))$ is SCC.
...
\begin{lemma}[Net point entropy gap]
  ...
  \begin{align*}
    \hNP(\mathrm{core}(\teich(V))) < \hNP(\mathrm{core}(\teich(\os)))
  \end{align*}
\end{lemma}
...
\begin{proof}
  ...
  \begin{align}
    \label{eq:ineq1}
    \hLP(\teich(\os)) \leq \hNP(\mathrm{core}(\teich(\os)))
  \end{align}
  ...
  \begin{align}
    \label{eq:eq3}
    \hLP(\teich(V)) = \hNP(\mathrm{core}(\teich(V)))
  \end{align}
...
\begin{proof}[Proof of Theorem \ref{thm:entropy-equality}]
  ...
  \begin{align*}
    \#\left( B_R(p) \cap \mathcal{N} \right) \leq q(R) \cdot \exp(\hLP(\teich(\os)) \cdot R \cdot (1 + 2 \vepb))
  \end{align*}
  ...
  \begin{align}
    ...
    \mathcal{N}_g(p, R, \vepb) = \bigsqcup_{\gamma \in \mcg(\os)} \mathcal{N}_g(\gamma, p, R, \vepb)
  \end{align}
  Observe that if $y \in \mathcal{N}_g(\gamma, p, R, \vepb)$, then $d(p, \gamma p) \leq R(1 + \vepb)$, since $d(p, y) \leq R$ and $d(y, \gamma p) \leq \vepb R$.
  This observation leads to the following upper bound on $\#\left( \mathcal{N}_g(p, R, \vepb) \right)$.
  \begin{align}
    \#\left( \mathcal{N}_g(p, R, \vepb) \right) \leq \sum_{\substack{\gamma \in \mcg(\os) \\ d(p, \gamma p) \leq R(1 + \vepb)}} \#\left( B_{\vepb R}(\gamma p) \cap \mathcal{N}_g(\gamma, p, R, \vepb)  \right) \label{eq:good-estimate-1}
  \end{align}
  By Lemma \ref{lem:fd-polynomial-growth}, there exists a polynomial $q(R)$ such that each term in the above sum is at most $q(R)$.
  \begin{align}
    \#\left( \mathcal{N}_g(p, R, \vepb) \right) &\leq \sum_{\substack{\gamma \in \mcg(\os) \\ d(p, \gamma p) \leq R(1 + \vepb)}} q(R) \\
  ...
  By Lemma \ref{lem:net-point-entropy-inequality}, we have that $\hNP(\mathrm{core}(\teich(V))) < \hNP(\mathrm{core}(\teich(\os)))$: this is precisely the hypothesis we need to apply Theorem \ref{thm:linear-gap-rescaled}.
  ...
  \begin{align*}
    \#\left( \mathcal{N}_b(p, R, \vepb) \right) \leq kR^k \cdot \exp((\hNP(\mathrm{core}(\teich(\os))) + \veperr) \cdot R \cdot (1 - c))
  \end{align*}
  We pick $\veperr$ small enough such that the above term satisfies the following inequality for large enough $R$.
  \begin{align*}
    \exp((\hNP(\mathrm{core}(\teich(\os))) + \veperr) \cdot R \cdot (1 - c)) < \exp\left(\hNP(\mathrm{core}(\teich(\os))) \cdot R \cdot \left(1 - \frac{2c}{3}\right)\right)
  \end{align*}
  On the other hand, we have that for large enough $R$, the total number of net points is at least $\exp\left(\hNP(\mathrm{core}(\teich(\os))) \cdot R \cdot \left(1 - \frac{c}{2}\right)\right)$.
...
From this volume estimate, we can obtain an estimate on the cardinality of net points of an $(\vepn, 2 \vepn)$-net $\mathcal{N}$.
...
\end{align}
Here $\mathcal{N}_1$ and $\mathcal{N}_2$ are $(\vepn, 2\vepn)$ nets for $\teich(S \setminus \gamma)$ and $\mathbb{H}$, and $B_R(p, S \setminus \gamma)$ and $B_R(p, \mathbb{H})$ are projections on the ball $B_R(p)$ to the two components.
...
We will also abuse notation slightly and use $\hNP(V)$ to refer to $\hNP(\mathrm{core}(\teich(V)))$ whenever $V$ is a non-orientable surface: when $V$ is orientable, $\hNP(V)$ will refer to $\hNP(\teich(V))$.
...
\end{example}
  ...
  However, this approach also leads to overcounting \todo{Replace overcounting with a different word?}, because of overlapping regions coming from nested or disjoint subsurfaces.
...
\begin{theorem}[Theorem 12.1 of \cite{dowdall2023lattice}]
  ...
  \begin{align*}
    \# \left( y \in \mathcal{N} \mid \mathfrak{L}(x,y) \leq r \right) \leq p(r) \cdot \exp\left( (1+\veperr)r \right)
  \end{align*}
\end{theorem}
...
\begin{rem}
  ...
  \begin{align*}
    \#\left( y \in \mathcal{N} \mid d_{\teich(V)}(x, y) \leq R \right) \leq \exp\left( (\hNP(V) + \vepent ) R \right)
  \end{align*}
\end{rem}
...
\begin{theorem}
  ...
  \begin{align*}
    \hNP(V) < \hNP(\os)
  \end{align*}
  Then for any $\vepb > 0$, there exists $c > 0$, and $R$ large enough, such that for any bad point $y$, i.e. a point in $\mathcal{N}_b(p, R, \vepb)$, the following upper bound holds for the complexity length between $p$ and $y$.
  \begin{align*}
