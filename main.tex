...
\begin{proof}
  ...
  \item $\os_{1,1,0}$: Since $\os_{1,1,0}$ is orientable, we have that $\mathrm{core(\teich(\os_{1,1,0}))} = \teich(\os_{1,1,0})$, so it suffices to look at the full Teichmüller space.
    ...
    \begin{align}
      ...
      \#\left( B_R(p) \cap N \right) = \#\left( B_R(p) \cap N_{\leq \varepsilon R} \right) + \#\left( B_R(p) \cap N_{> \varepsilon R} \right)
    \end{align}
    Here, $N_{\leq \varepsilon R}$ denotes the net points that lie within distance $\varepsilon R$ of the orbit of $p$, and $N_{> \varepsilon R}$ denotes the net points that lie more than distance $\varepsilon R$ of the orbit of $p$.
    ...
    Let $N_{\leq \varepsilon R}(\gamma)$ denote the subset of $N_{\leq \varepsilon R}$ whose closest lattice point is $\gamma p$.
    ...
    \begin{align*}
      \#\left( B_R(p) \cap N_{\leq \varepsilon R} \right) \leq p(R)
    \end{align*}
    Using the two facts we stated, we get the following upper bound for $\#\left( B_R(p) \cap N_{\leq \varepsilon R} \right)$.
    \begin{align*}
      \#\left( B_R(p) \cap N_{\leq \varepsilon R} \right) \leq p(R) \cdot \left( \exp(R(1+\varepsilon)) \right)
    \end{align*}
    ...
    For any point $x$ in $N_{> \varepsilon R}$, we can decompose the geodesic $[p, x]$ into two segments $[p, x_0]$, and $[x_0, x]$, where $x_0$ is the last point on the geodesic where it stays within some bounded distance of a lattice point. \todo{Fix this}
    ...
    We thus have the following upper bound on $\#\left( B_R(p) \cap N_{> \varepsilon R} \right) $ for large enough values of $R$.
    \begin{align*}
      \#\left( B_R(p) \cap N_{> \varepsilon R} \right)  &\leq p(R) \cdot \exp(R(1-\varepsilon)) \\
...
\begin{definition}[Good points]
  A point in $B_R(p) \cap N$ is good if it is at most distance $\varepsilon R$ away from a lattice point $\gamma p$. The set of good points is denoted by $N_g(p, R, \varepsilon)$.
\end{definition}

\begin{definition}[Bad points]
  A point in $B_R(p) \cap N$ is bad if it is more than distance $\varepsilon R$ away from the nearest lattice point. The set of bad points is denoted by $N_b(p, R, \varepsilon)$.
\end{definition}
...
We also further subdivide $N_g(p, R, \varepsilon)$ based on what the closest lattice point is.

\begin{definition}[Good point in the domain of $\gamma$]
  For $\gamma \in \mcg(S)$, the set $N_g(\gamma, p, R, \varepsilon)$ denotes the subset of $N_g(p, R, \varepsilon)$ whose closest lattice point is $\gamma p$.
\end{definition}
...
\begin{lemma}
  ...
  \begin{align*}
    \#\left( N_g(\gamma, p, R, \varepsilon) \cap B_R(\gamma p) \right) \leq q(R)
  \end{align*}
\end{lemma}
...
\begin{lemma}[Packing bound]
  ...
  For an constants $C > 0$ and $\varepsilon > 0$, there exists a constant $D(C, \varepsilon, S)$ depending on the constants $C$, $\varepsilon$, and the topological type of the surface $S$ such that any ball $B_C(p)$ in $\teich(S)$ cannot contain more than $D$ points that are pairwise distance at least $\varepsilon$ apart.
\end{lemma}
...
\begin{proof}[Proof of Lemma \ref{lem:fd-polynomial-growth}]
  ...
  \end{align}
  Here, $N(\gamma)$ denotes the set of net points whose closest lattice point is $\gamma p$: $N(\gamma)$ is therefore a superset of $N_g(\gamma, p, R, \varepsilon)$.
...
Since $\# \cZ \leq R^{f(\chi(S))}$, we have that $\#\left( N(1) \cap B_p(R) \right) \leq C^{\prime} R^{f(\chi(S))}$, for some other constant $C^{\prime}$, by Lemma \ref{lem:packing-argument}.
...
\begin{theorem}[Linear gap in complexity length]
  ...
  \end{align*}
  Then for any $\varepsilon >0$, there exists $c > 0$, such that for any bad point $y$, i.e. a point in $N_b(p, R, \varepsilon)$, the following upper bound on the complexity distance between $p$ and $y$ holds.
  \begin{align*}
    \dcomp(p, y) \leq R(1 - c)
  \end{align*}
\end{theorem}
...
\begin{proof}[Proof of Theorem \ref{thm:entropy-equality}]
  ...
  \begin{align}
    ...
    N_g(p, R, \varepsilon) = \bigsqcup_{\gamma \in \mcg(S)} N_g(\gamma, p, R, \varepsilon)
  \end{align}
  Observe that if $y \in N_g(\gamma, p, R, \varepsilon)$, then $d(p, \gamma p) \leq R(1 + \varepsilon)$, since $d(p, y) \leq R$ and $d(y, \gamma p) \leq \varepsilon R$.
  This observation leads to the following upper bound on $\#\left( N_g(p, R, \varepsilon) \right)$.
  \begin{align}
    \#\left( N_g(p, R, \varepsilon) \right) \leq \sum_{\substack{\gamma \in \mcg(S) \\ d(p, \gamma p) \leq R(1 + \varepsilon)}} \#\left( B_{\varepsilon R}(\gamma p) \cap N_g(\gamma, p, R, \varepsilon)  \right) \label{eq:good-estimate-1}
  \end{align}
  By Lemma \ref{lem:fd-polynomial-growth}, there exists a polynomial $q(R)$ such that each term in the above sum is at most $q(R)$.
  \begin{align}
    \#\left( N_g(p, R, \varepsilon) \right) &\leq \sum_{\substack{\gamma \in \mcg(S) \\ d(p, \gamma p) \leq R(1 + \varepsilon)}} q(R) \\
  ...
  \begin{align*}
    \#\left( N_b(p, R, \varepsilon) \right) \leq kR^k \cdot \exp(\hNP(\mathrm{core}(\teich(S))) \cdot R \cdot (1 - c))
  \end{align*}
