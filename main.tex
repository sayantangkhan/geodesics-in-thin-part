...
\begin{definition}[Entropy for bad lattice points]
  ...
  \end{align*}
  The entropy for bad lattice points $\hLPb(\core(\teich(\no_g)), \vept)$ is the following quantity.
  \begin{align*}
...
\begin{proof}
  ...
  We turn this geodesic segment into a trajectory by marking off points at distance $\tau\left( 1 - \frac{\vepn}{\tau} \right)$ on the segment, and then replacing those points with the nearest net point.
...
In this section, we define the volume form $\nu_N$ we use for $\teich(\no_g)$, and as an application of Theorem \ref{thm:prno}, we show that the for balls of fixed radius in $\core(\teich(\no_g))$, the $\nu_N$ volume of the ball is bounded above and below by constants that are independent of the center of the ball.

Let $\mathcal{P}$ be a pants decomposition for $\no_g$: \textcite{norbury2008lengths} defined a volume form on $\teich(\no_g)$ using the pants decomposition, analogous to the Weil-Petersson volume form on Teichmüller spaces of orientable surfaces.
