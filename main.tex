...
\begin{definition}[Complete witness family]
  For an active subsurface $V$, a witness $W$ is said to be the $\Omega$-completion of $V$, denoted $\overline{V}$ \todo{Figure out how Dowdall-Masur get a subscript on the overline.} if $W$ is the minimal (by inclusion) witness containing $V$.
...
\end{example}
  ...
  Then the realization theorem (Theorem \ref{thm:consistency-realization}) will give us a point $\widehat{w}_V^{\Omega}$ in $\teich(V)$ which has the same projections $\cC(Z)$ as the original point $W$.
  ...
  \begin{definition}[Resolution point]
    ...
    \begin{itemize}
    \item If $U$ is non-annular, then $\widehat{w}_U^{\Omega} \in \teich(U)$ is the thick point whose projections to $\cC(V)$ for $V \sqsubseteq U$ are coarsely equal to the projection tuple $\widehat{w}_U$.
  ...
  \begin{definition}[Complexity of witness family]
    ...
    The complexity $\mathfrak{L}(x,y)$ of $\Omega$ is the following quantity.
    \begin{align*}
      \mathfrak{L}(x,y) \coloneqq \sum_{U \in \Omega} \hNP^{\ast}(U) \cdot d_{\teich(U)}(\widehat{x}_{U}^{\Omega}, \widehat{y}_{U}^{\Omega})
    \end{align*}
...
{\color{red} Go back to the conjugation example, and see what complexity length it gets assigned.}
...
\item {\color{red} DONE:} Make precise the notion of product region being active.
\item {\color{red} DONE:} We deal with the overcounting by constructing witness families, i.e. subsets of subsurfaces, which are minimal in an appropriate sense.
\item {\color{red} DONE:} We can use these witness families to resolve points on the geodesics to points in the Teichmüller space of subsurfaces. We need some sort of alignment for fellow traveling.
\item {\color{red} DONE:} We specify properties of witnesses that we need to resolve points.
\item {\color{red} DONE:} The define complexity length.
