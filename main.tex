...
\begin{proof}
  ...
  \item $\os_{1,1,0}$: Since $\os_{1,1,0}$ is orientable, we have that $\mathrm{core(\teich(\os_{1,1,0}))} = \teich(\os_{1,1,0})$, so it suffices to look at the full Teichmüller space.
    ...
    For any point $x$ in $N_{> \varepsilon R}$, we can decompose the geodesic $[p, x]$ into two segments $[p, x_0]$, and $[x_0, x]$, where $x_0$ is the last point on the geodesic where it stays within some bounded distance of a lattice point.
...
In this section we introduce an alternative to the Teichmüller metric, called the \emph{complexity length}.
...
All the results we state about complexity length will involve a arbitrary large parameter $C \gg 0$: for specific applications, we will need to set $C$ larger than some threshold value which we will specify.
