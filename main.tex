...
\title{Statistical convex-cocompactness for Teichmüller spaces of non-orientable surfaces}
...
\item Behrstock and BGI inequalities: really the whole HHS package.
...
Use Behrstock to show that progress on subsurfaces that don't come from lifts is negligible.

\todo[inline]{Write down the key idea of all the proofs in this section, namely, going to the double cover, and getting an isometric embedding in the Teichmüller case and the curve complex case \textcite{masur2013geometry}}

\begin{theorem}[Isometric embedding of Teichmüller spaces]
  \label{thm:i-embedding-teich-space}
  Blah
\end{theorem}

\begin{theorem}[Quasi isometric embedding of curve complexes \cite{masur2013geometry}]
...
The main tool we will need to prove that the weak convex core is minimally distorted is a version of Minsky's product region theorem \cite[Theorem 6.1]{1077244446} for non-orientable surfaces.
...
We define a metric on the product $X_\gamma$ as the supremum of the metrics on each of the components, where the metric on $\teich(\no_g \setminus \gamma)$ is the Teichmüller metric, the metric on the $\mathbb{H}_i$ components is the hyperbolic metric, and the metric on the $(\mathbb{R}_{>0})_i$ is given by $d(x,y) = \left| \log\left( \frac{x}{y}  \right) \right| $.
...
\begin{proof}
  ...
  \item $\no_g \setminus \gamma$: The lift of $\no_g \setminus \gamma$ to $\os_{g-1}$ will have two components if $\no_g \setminus \gamma$ is orientable, which we call $S_1$ and $S_2$. Both $S_1$ and $S_2$ are homeomorphic to $\no_g \setminus \gamma$.
    ...
    In the second case, we have that $\teich(\no_g \setminus \gamma)$ also isometrically embeds inside the Teichmüller space of its double cover, so the claim follows.
...
\subsection{Distance formula for Teichmüller space}
...
Finally, let $\cC^+$ and $\cC^-$ denote the set of two-sided and one-sided curves on $\no_g$.
...
\begin{proof}
 ...
 There are three possibilities for $Y$ in $\os_{g-1}$.
 \begin{enumerate}[(i)]
 \item $Y$ is one component of a lift of an orientable subsurface $Z$ of $\no_g$: In this case $Y$ and $\iota(Y)$ are disjoint, and the restriction of the covering map to $Y$ (or $\iota(Y)$) is a homeomorphism.
   We then have $d_Y(\wt{\mu_x}, \wt{\mu_y}) = d_Z(\mu_x, \mu_y)$ (and the same equality with $Y$ replaced with $\iota(Y)$).
 ...
 \item $Y$ is the lift of a non-orientable subsurface $Z$ of $\no_g$: In this case, we can apply Theorem \ref{thm:qi-embedding-curve-complex} to get that $d_Y(\wt{\mu_x}, \wt{\mu_y}) \emul d_Z(\mu_x, \mu_y)$.
 \item $Y$ is not a lift of a subsurface of $\no_g$: If $Y$ is not a lift of a subsurface from $\no_g$, that means $Y$ and $\iota(Y)$ are not disjoint, nor are they equal.
   Since one of them can't be nested in the other, they must be transverse.
   By the Behrstock inequality for transverse surfaces, there exists some $k_0$ large enough such that the following holds for any $x$, $y$, and $Y$.
   \begin{align}
     \min\left( d_Y(\wt{\mu_x}, \wt{\mu_y}), d_{\iota(Y)}(\wt{\mu_x}, \wt{\mu_y}) \right) \leq k_0
     \label{ineq:behrstock-transverse}
   \end{align}
   Also note that $d_Y(\wt{\mu_x}, \wt{\mu_y}) = d_{\iota(Y)}(\wt{\mu_x}, \wt{\mu_y})$, since $\iota(\wt{\mu_i}) = \wt{\mu_i}$ for $i = x$ and $i=y$.
   Plugging this into \eqref{ineq:behrstock-transverse}, we get a bound on the curve complex projection to $Y$.
   \begin{align*}
...
\subsection{Curve complex results}
\label{sec:curve-compl-results}
