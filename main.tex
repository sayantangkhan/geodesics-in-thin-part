...
% \begin{align*}
%   % d_{\teich(\os)}(q, y) &> \varepsilon_3 R \\
%   d_{\teich(\os)}(p, q) &\leq (1-\varepsilon_3) R
% \end{align*}
...
% \begin{align}
%   \label{eq:eq1}
%   % d_{\teich(\os)}(\widehat{y}_{\os}^{\Omega}, y) &> \varepsilon_3 R - J \\
%   d_{\teich(\os)}(p, \widehat{y}_{\os}^{\Omega}) &\leq (1-\varepsilon_3) R + J
% \end{align}
...
We next estimate $\mathfrak{L}(\widehat{y}_{\os}^{\Omega})$: we claim that there exists a WISCAL witness family $\Omega^{\prime}$ for $[\widehat{y}_{\os}^{\Omega}, y]$ that does not have $\os$ as a witness.
...
Since we have from our hypothesis that $\hNP(\mathrm{core}(\os)) > \hNP(\mathrm{core}(S^{\prime}))$, there exists a constant $h$ such that $h < \hNP(\mathrm{core}(\os))$ but $h > \hNP(\mathrm{core}(S^{\prime}))$.
