...
\begin{proof}
  ...
  \item $\os_{1,1,0}$: Since $\os_{1,1,0}$ is orientable, we have that $\mathrm{core(\teich(\os_{1,1,0}))} = \teich(\os_{1,1,0})$, so it suffices to look at the full Teichmüller space.
    ...
    Note that this reduces to estimating the volume of the intersection of a ball $B_R(x_0)$ with a set $H$, where $H$ is the fundamental domain for the action of a parabolic element on a horoball which lies distance $\vepb R$ from $x_0$.
    Working in the upper half plane model for $\mathbb{H}$, where $x_0 = i$, and the region $H$ is the set of points whose imaginary component is greater than $\exp(\vepb R)$, we get that the region of integration is a rectangle, bounded by $0 \leq \mathrm{Re}(z) \leq 1$ and $\exp(\vepb R) \leq  \mathrm{Im}(z) \leq \exp(R)$.
    The volume of this region is given by the following integral.
    \begin{align*}
      \mathrm{Vol}(B_R(x_0) \cap H) &\leq \int_{\exp(\vepb R)}^{\exp(R)} \int_0^1 \frac{1}{y^2} dx dy \\
                                    &= \exp(-\vepb R) - \exp(-R) \\
      &\leq \exp(-\vepb R)
    \end{align*}
