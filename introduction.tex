...
This means pseudo-Anosov elements are \emph{strongly contracting}.
...
\begin{theorem}[Exponential genericity of pseudo-Anosovs (Theorem 1.4 of \cite{yang2020genericity})]
  \label{thm:pure-exponential}
  For a point $p \in \systole(\no_g)$, let $N(R)$ denote the number of mapping class elements $\gamma$ such that $d(p, \gamma p) \leq R$, and let $N_{nc}(R)$ denote the non pseudo-Anosov mapping class elements $\gamma$ such that $d(p, \gamma p) \leq R$.
