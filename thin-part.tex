...
The above argument also shows that the number of net points in a ball of radius $R$ is (up to multiplicative errors) equal to the Norbury measure $\nu_N$.
...
\begin{definition}[Foster-Lyapunov-Margulis function]
  \label{defn:flm}
  A proper function $f$ on $\core(\teich(\no_g))/\mcg(\no_g)$ is called a Foster-Lyapunov-Margulis function if, if there exists a compact subset $W_0$ of $\core(\teich(\no_g))$, and functions $b(x)$ and $c(x)$, such that $A_{\tau}f$ satisfies the following inequality.
  \begin{align*}
    (A_{\tau}f)(x) \leq c(x) f(x) + b(x)
  \end{align*}
  Furthermore, $b(x)$ is a bounded function that is supported within a compact set $W_0$, and $c(x)$ satisfies the following inequality for all $x$ outside of $W_0$ for some polynomial $p$.
  \begin{align*}
    c(x) \leq \cdot p(\tau) \cdot \exp(-\tau)
  \end{align*}
...
\begin{proof}
  ...
  \item[-] $R_1$: The subset $R_1$ is defined in the following manner.
    \begin{align*}
      R_1 \coloneqq \left\{ x \mid \text{Shortest curve for any $z \in B_{\tau}^{\vept}(x)$ is $\gamma$ and $\ell(\gamma) < \delta$} \right\}
    \end{align*}
    This is the set of points $x$ such that there exists a unique curve $\gamma$ which is the shortest curve at all points in $B_{\tau}^{\vept}(x)$, and the length of $\gamma$ is less than $\delta$ for all points in $B_{\tau}^{\vept}(x)$.
  \item[-] $R_2$: This subset is $R_0 \setminus R_1$, where $R_0$ is defined in the following manner.
    \begin{align*}
      R_0 \coloneqq \left\{ x \mid \text{Shortest curve for any $z \in B_{\tau}^{\vept}(x)$ shorter than $\delta$} \right\}
    \end{align*}
...
\begin{definition}[\Concave trajectories]
  A trajectory $\left( r_0, r_1, \ldots, r_{n-1} \right)$ is said to be \concave if all the points in the trajectory except the first point and the last point are outside $\thick(\no_g)$.
\end{definition}
Since we picked $\vept$ smaller than $\delta$ that appears in the proof of Proposition \ref{prop:flm-is-flm}, for any of the \concave trajectory points $r_i$, we will get the following decay estimate for $A_{\tau}\flm(r_i)$.
\begin{align*}
...
\begin{proposition}
  ...
  For any $\veperr > 0$, there exists a $\vept^{\prime} > 0$ and $\vept^{\prime} > \vept > 0$, a constant $C^{\prime}$, and a large enough $R$, such that the following estimate holds for the counting function $M_{r_0}(R)$.
  \begin{align*}
